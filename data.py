...
def format_imagenette2(classes_json: Path) -> Tuple[Path, list[str]]:
...
def load_imagenette2(
    classes: list[str], batch: int, transforms: nn.Module
...
def load_CIFAR(
    train: bool,
    batch_size: int,
    max_batches_n: int | None = None,
    shuffle: bool = True,
    transforms: nn.Module | None = None,
    seed: int | None = None,
) -> Tuple[torchvision.datasets.CIFAR10, DataLoader]:
    ds = torchvision.datasets.CIFAR10(
        str(CIFAR_PATH), download=True, train=train, transform=transforms
    )
